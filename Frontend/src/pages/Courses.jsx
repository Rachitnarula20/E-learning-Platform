...
// ─── HELPER ────────────────────────────────────────────────────────────────
// Build a full URL for any media path (images or videos) returned by your API
const buildMediaUrl = (p = "") => {
  // 1) normalize Windows back-slashes to forward slashes
  const clean = p.replace(/\\/g, "/");
  // 2) if it's already a full URL, return it
  if (clean.startsWith("http")) return clean;
  // 3) ensure a leading slash
  const withSlash = clean.startsWith("/") ? clean : `/${clean}`;
  // 4) prepend the server base
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        ...
        >
          ...
          {[...new Set(courses.map(c => c.category))].map((category, i) => (
  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow p-4 relative">
      ...
      <div className="w-full h-48 overflow-hidden rounded-md mb-4">
        <img
          src={buildMediaUrl(course.image)}
          alt={course.title}
